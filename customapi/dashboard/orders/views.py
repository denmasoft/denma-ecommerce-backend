...
class ResetOrdersView(ListView):
    def get(self, request, *args, **kwargs):
        users = User.objects.all()
        for user in users:
            if user.username != 'admin' and user.username != 'yalint86@gmail.com':
                user.delete()
        ...
        for review in reviews:
            review.delete()
        messages.info(self.request, _("Dashboard cleaned successfully."))
...
class OrderListView(BulkEditMixin, ListView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        self.base_queryset = queryset_orders_for_user(
            request.user).order_by('-date_placed').filter(status='completed')
...
class OrderDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        tax_value = float(self.object.total_incl_tax) * float(settings.TAX)
        tax_value = round(tax_value, 2)
        ctx['tax_value'] = tax_value
        ctx['tax'] = tax
        basket_total = (float(self.object.total_excl_tax) + float(self.object.shipping_excl_tax)) - float(self.object.shipping_excl_tax)
        ctx['basket_total'] = basket_total
        order_total = float(self.object.total_excl_tax) + float(self.object.shipping_excl_tax) + tax_value
