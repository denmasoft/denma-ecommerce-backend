...
class PaymentExecute(OrderPlacementMixin, APIView):
    def get(self, request, pk=None, format=None):
        ...
        if payment.execute({"payer_id": payerid}):  # return True or False
            ...
            total = float(order.total_incl_tax) + float(order.shipping_incl_tax) + float(tax_value)
            msg_html = render_to_string('invoice.html', {
                ...
                'tax_value': tax_value})
            notify_html = render_to_string('notification.html', {
            'total': total,
            'items': cart_ops.getitems(order),
            'discounts': cart_ops.getdiscounts(order),
            'shippings': cart_ops.getShipping(order),
            ...
            'wholesale': license})
            self.freeze_basket(order.basket)
            if order.shipping_code != 0:
                transaction = shippo.Transaction.create(rate=order.shipping_code, async=False)
                if transaction.status == "SUCCESS":
                    self.checkout_session.set_submitted_basket(order.basket)
                    order.basket.submit()
                    order.status = 'completed'
                    order.save()
                    send_mail('Thanks for purchasing with us', msg_html, settings.DEFAULT_FROM_EMAIL, [to_email],
                              fail_silently=False, html_message=msg_html)
                    send_mail('Purchase Information', notify_html, settings.DEFAULT_FROM_EMAIL,
                              [settings.CONTACT_EMAIL],
                              fail_silently=False, html_message=notify_html)
                ...
                send_mail('Thanks for purchasing with us', msg_html, settings.DEFAULT_FROM_EMAIL, [to_email],
                          fail_silently=False, html_message=msg_html)
                send_mail('Purchase Information', notify_html, settings.DEFAULT_FROM_EMAIL, [settings.CONTACT_EMAIL],
                          fail_silently=False, html_message=notify_html)
                return redirect(settings.FRONT_URL + "/" + payment.id)
        else:
            print(payment.error)
...
class StripeChargeView(APIView):
    ...
    def post(self, request, pk=None, format=None):
        ...
        send_mail('Thanks for purchasing with us', "You have purchased successfully", from_email, [to_email],
                  fail_silently=False)
        return redirect(settings.FRONT_URL)


class StripeView(OrderPlacementMixin, APIView):
    def get(self, request, pk=None, format=None):
        stripe.api_key = settings.STRIPE_API_KEY
...
class CartCheckoutView(BasketPermissionMixin, views.APIView):
    ...
    def post(self, request, format=None):
        ...
        #basket_id = cart_ops.get_basket(request.data['token'], request)
        try:
            order = Order.objects.get(basket=checkout_data['basket'])
            order.delete()
        except ObjectDoesNotExist:
            pass
        ...
        shipping_address_line4 = checkout_data['shipping_address_line4']
        ...
        delivery_address_line4 = checkout_data['delivery_address_line4']
        ...
        basket_data = {
            "basket": checkout_data['basket_url'],
        ...
        if c_ser.is_valid():
            order = c_ser.save()
            order.shipping_code = shipping_method_id
            ...
            order.total_excl_tax = float(order.total_excl_tax)
            order.total_incl_tax = float(order.total_incl_tax)
            if order.user is not None:
                try:
                    wholesale = WholeSale.objects.get(user=order.user)
                    if wholesale:
                        ototal = cart_ops.order_total(order)
                        order.total_excl_tax = float(ototal)
                        order.total_incl_tax = float(ototal)
                except ObjectDoesNotExist:
                    ''
            order.save()
            #basket.freeze()
...
class CustomBasketView(APIView):
    ...
    def get(self, request, format=None):
        if 'basket' in request.data:
            bask = request.data['basket']
            if bask:
                basket = get_object_or_404(Basket, pk=bask)
                basket = operations.prepare_basket(basket, request)
        else:
...
class CartShippingRates(APIView):
    ...
    def post(self, request, format=None):
        params = request.data
        total = params['total']
        result = [{"description": "Flat Rate", "image": "", "amount": "13.50", "provider": "SBS", "attributes": ["FLAT"],
             "name": "FLAT RATE", 'object_id': 0}]
        if int(params['zip']) == int(settings.ZIP_CODE):
            result.append({"description": "Local Shipping (One Day)", "image": "", "amount": "9.99", "provider": "SBS", "attributes": ["Local Shipping"],
             "name": "Local Shipping (One Day)", 'object_id': 0})
        if float(total) >= float(settings.FREE_SHIPPING_OVER):
            result = [{"description": "Free Shipping Rate", "image": "", "amount": "0.00", "provider": "SBS",
                        "attributes": ["FREE SHIPPING FLAT"],
                        "name": "FREE SHIPPING FLAT RATE", 'object_id': 0}]
        # AQUI----
...
class ResetOrders(APIView):
    ...
    def get(self, request, format=None):
        ...
        baskets = baskets.filter(Q(status="frozen") | Q(status="open"))
